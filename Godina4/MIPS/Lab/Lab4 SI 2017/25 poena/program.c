...
//LCD Displej - LM020L (HD44780)
#define ADR_LCD_INST			0x0002
#define ADR_LCD_DATA			0x0006
...
void sendLCD(Word16 addr, Byte8 data)
{
	while(inp(ADR_LCD_INST) & CTRL_LCD_BUSY_FLAG);
	outp(addr, data);
}
...
void writeLcd() {
	int i;
	sendLCD(ADR_LCD_INST, 0x80);
	for (i = 0; i < 16; i++) {
		if (i < msg_cnt) sendLCD(ADR_LCD_DATA, msg[i]);
		else sendLCD(ADR_LCD_DATA, ' ');
	}
}
...
void initLcd() {
	sendLCD(ADR_LCD_INST, CTRL_LCD_INST_DISP_ON_OFF | CTRL_LCD_DISP_ON | CTRL_LCD_CURS_OFF | CTRL_LCD_BLINK_OFF);
	sendLCD(ADR_LCD_INST, CTRL_LCD_INST_FUNCTION_SET | CTRL_LCD_8_BIT | CTRL_LCD_1_LINE | CTRL_LCD_5x7_DOTS);
	sendLCD(ADR_LCD_INST, CTRL_LCD_INST_ENTRY_MODE | CTRL_LCD_INC_ADR | CTRL_LCD_SHIFT_OFF);
}
